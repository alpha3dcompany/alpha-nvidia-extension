        ...
        with self._window.frame:
            if self._show_login:
                with ui.VStack(spacing=4):
                    ...
                    label_login_error = ui.Label(self._show_login_error_msg, alignment=ui.Alignment.CENTER_TOP, height=20, style={"color": "red"}) if self._show_login_error else None


                    email_label = ui.Label("Email:", alignment=ui.Alignment.CENTER_BOTTOM, height=10)
                    email_input = ui.StringField(placeholder="Email", height=20, style={"margin_width": 40})

                    password_label = ui.Label("Password:", alignment=ui.Alignment.CENTER_BOTTOM, height=30)
                    password_input = ui.StringField(password_mode=True, height=20, style={"margin_width": 40})

                    ui.Button("Log In", height=80, style={"margin_width": 100, "margin_height": 20})

            else:
                with ui.VStack():
                    label_assets_error = ui.Label(self._show_assets_error_msg, alignment=ui.Alignment.CENTER_TOP, height=20, style={"color": "red"}) if self._show_assets_error else None
                        ...
                        with ui.VGrid(column_width=100, column_height=100):
                            for i in range(10):
                                ...
                                def drag_area(thumbnail, url):
                                    ...
                                    brand_name = ui.Label("Brand name", alignment=ui.Alignment.CENTER_TOP, height=10, style={"font_size": 15})
                                    model_name = ui.Label("Model name", alignment=ui.Alignment.CENTER_TOP, height=30, style={"font_size": 15})
                                    image.set_drag_fn(lambda: drag(url, thumbnail))

                                with ui.VStack():
                                    drag_area("https://www.clipartkey.com/mpngs/m/269-2698741_clip-art-texture-png-for-bush-tree-png.png", "http://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Vegetation/Shrub/Acacia.usd")
